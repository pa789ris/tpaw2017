        ...
        if ($("#Nom").val().length<5){
           ...
           $('.modal').modal('show');     //mettre focus sur le champ d'erreur)
             ...
             $('.modal-title').html(" Entrer votre mot de passe  Mr "+$("#Nom").val());
               $(".modal-body").html('<img src="https://maps.googleapis.com/maps/api/staticmap?center='+$("#adr")+'&zoom=13&size=300x300&maptype=roadmap&markers='+$("#adr")+'"/>');//mettre mr et extraire nom
           $('.modal').modal('show');       //mettre focus sur le champ d'erreur)
             //faire teste pour la date
       }
         else {
