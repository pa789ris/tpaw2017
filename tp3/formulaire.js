        ...
        if ($("#Nom").val().length<5){
           ...
           $('.modal').modal('show');     //mettre focus sur le champ d'erreur)
             ...
             $('.modal-title').html(" Bienvenue   "+$("#Nom").val());
                  $('.modal-body').html('<p>vous etes nee le    ' +$("#datepicker").val()+ ' et vous habitez : <br/> <a href="https://developers.google.com/maps/documentation/static-maps/"> <img src="https://maps.googleapis.com/maps/api/staticmap?center=' + $("#adr").val() + '&markers=' + $("#adr").val() + '&size=640x400"/> </a> </p>')
           $('.modal').modal('show');       //mettre focus sur le champ d'erreur)
        }
    });
});
