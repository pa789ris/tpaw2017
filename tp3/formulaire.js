        ...
        if ($("#Nom").val().length<5){
           ...
           $('.modal').modal('show');     //mettre focus sur le champ d'erreur)
             ...
             $('.modal-title').html(" Bienvenue   "+$("#Nom").val());
                  $('.modal-body').html('<p>vous etes neee le    ' +$("#datepicker").val()+ ' et vous habitez : <br/> <a href="https://developers.google.com/maps/documentation/static-maps/" target="_blank"> <img src="paris.png"/> </a> </p> <br/>+"5"')
            $('.modal').modal('show');
           $('.modal').modal('show');       //mettre focus sur le champ d'erreur)
        }
    });
